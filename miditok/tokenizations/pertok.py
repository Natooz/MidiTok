...
from miditok.classes import TokenizerConfig, Event, TokSequence
from miditok.midi_tokenizer import MusicTokenizer 
from miditok.constants import MIDI_INSTRUMENTS
from symusic import Score, Track, Note, TimeSignature, TimeUnit


class PerTok(MusicTokenizer):
    r"""
    PerTok (Performance Tokenizer) is designed to capture the full spectrum of rhythmic values (16ths, 32nds,
    various denominations of triplets/etc.) in addition to velocity and microtiming performance characteristics.
    ...
    *'Timeshift' tokens are expressed as the quantized time value, as one would expect in a sheet music score. 
    E.g. at 480 ticks per quarter note, a 16th note would be expressed as 'Timeshift_120'
    The microtiming shift is then characterized as the subtle deviation from this quantized value.
    E.g. for a slightly delayed 1/8th note:
    'Timeshift_240', 'Pitch_60', 'Velocity_100', 'Microtiming_12'
    
    Additionally, 'Bar' tokens are inserted at the start of each new measure. This helps further reduce
    seq. length and theoretically reduces the 'timing drift' models can develop at longer seq. lengths.
    ...
    granularity: Granularity of rhythm as measured per quarter note, e.g. 480 ticks-per-quarter (tpq)
    ...
    config = TokenizerConfig(
        ...
        special_tokens=["PAD", "BOS", "EOS", "MASK", "Bar"],
        beat_grids=[(0, 8, 4), (0, 8, 6)],
        use_duration=True,
        use_velocity=True,
        use_microtiming=True,
        granularity = 480,
        max_microtiming_shift = 50,
    ...
    def _tweak_config_before_creating_voc(self) -> None:
        ...
        for key in ['beat_grids', 'use_duration', 'use_velocity', 'use_microtiming',
             'granularity', 'max_microtiming_shift', 'num_microtiming_bins']:
            
            assert key in self.config.additional_params, f"{key} must be specified in tokenizer config."
                     
        assert self.config.additional_params['max_microtiming_shift'] >= \
            self.config.additional_params['num_microtiming_bins'], \
            "Max microtiming shift value must be >= n. microtiming bins"
    ...
    def create_timeshift_tick_values(self):
    ...
    def preprocess_score(self, score: Score) -> Score:
        ...
        if len(score.time_signatures) == 0:
            score.time_signatures.append(TimeSignature(time=0, numerator=4, denominator=4))#, ttype=TimeUnit.tick))

        return score
    
    def get_closest_array_value(self, value, array):
    ...
    def _create_track_events(self, track: Track, _: None = None) -> list[Event]:
        ...
        **If the tokenizer is using pitch intervals, the notes must be sorted by time
        ...
        for note in track.notes:
            ...
            # Check if next notes time is in another bar; if so, add 'bar' tokens and keep advancing clock
            ...
            closest_timeshift = int(
                    self.get_closest_array_value(
                        value=time_delta, 
                        array=self.timeshift_tick_values)
                )
            ...
            if self.use_microtiming:
                ...
                closest_microtiming = int(
                        self.get_closest_array_value(
                        value=microtiming,
                        array=self.microtiming_tick_values)
                    )
                events.append(
                    Event(
                        type_="MicroTiming",
    ...
    def _score_to_tokens(self, score: Score) -> TokSequence or list[TokSequence]:
            ...
            for ti, token in enumerate(seq):
                ...
                if token_type == "Bar":
                    ...
                    try:
                        ...
                        # Duration
                        if self.use_duration and (seq[counter+1].split("_")[0] == "Duration"):
                            counter += 1
                        ...
                        # Velocity
                        if self.use_velocity and seq[counter+1].split("_")[0] == "Velocity":
                            counter += 1
                        ...
                        # Microtiming
                        if self.use_microtiming and seq[counter+1].split("_")[0] == "MicroTiming":
                            counter += 1
    ...
    def _create_base_vocabulary(self) -> list[str]:
        ...
        #Timeshift
        vocab += [
            f'TimeShift_{(str(timeshift))}' for timeshift in self.timeshift_tick_values
        ]
        
        if self.use_duration:
            vocab += [
                f'Duration_{(str(timeshift))}' for timeshift in self.timeshift_tick_values
            ]
        
        #Microtiming
        if self.use_microtiming:
            max_mt = self.config.additional_params['max_microtiming_shift']
            mt_bins = self.config.additional_params['num_microtiming_bins']
            
            self.microtiming_tick_values = np.linspace(
                -max_mt, max_mt, mt_bins + 1, dtype=np.intc
            )
            
            vocab += [
                f'MicroTiming_{str(microtiming)}' for microtiming in self.microtiming_tick_values
            ]
    ...
    def _add_time_events(self, events: list[Event], time_division: int):
        pass
