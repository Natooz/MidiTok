...
class MuMIDI(MIDITokenizer):
    ...
    def __init__(self, pitch_range: range = PITCH_RANGE, beat_res: Dict[Tuple[int, int], int] = BEAT_RES,
        ...
        self.vocab_types_idx = {'Pitch': 0, 'DrumPitch': 0, 'Position': 0, 'Bar': 0, 'Program': 0, 'BarPosEnc': 1,
                                'PositionPosEnc': 2, 'Velocity': -2, 'Duration': -1}
    ...
    def midi_to_tokens(self, midi: MidiFile, *args, **kwargs) -> List[List[int]]:
        ...
        if self.max_bar_embedding < nb_bars:
            self.vocab[1].add_event(f'BarPosEnc_{i}' for i in range(self.max_bar_embedding, nb_bars))
        ...
        for note_token in note_tokens:
            ...
            if note_token[0].time != current_tick:
                ...
                if current_bar < current_tick // ticks_per_bar:
                    ...
                    for i in range(nb_new_bars):
                        bar_token = [self.vocab[0].event_to_token['Bar_None'],
                                     self.vocab[1].event_to_token[f'BarPosEnc_{current_bar + i + 1}'],
                                     self.vocab[2].event_to_token['PositionPosEnc_None']]
                        if self.additional_tokens['Tempo']:
                            bar_token.append(self.vocab[self.vocab_types_idx['Tempo']][f'Tempo_{current_tempo}'])
                ...
                if self.additional_tokens['Tempo']:
                    pos_token.append(self.vocab[self.vocab_types_idx['Tempo']].event_to_token[f'Tempo_{current_tempo}'])
            ...
            if note_token[0].desc != current_track:
                ...
                track_token = [self.vocab[0].event_to_token[f'Program_{current_track}'],
                               ...
                               self.vocab[2].event_to_token[f'PositionPosEnc_{current_pos}']]
                if self.additional_tokens['Tempo']:
                    track_token.append(self.vocab[self.vocab_types_idx['Tempo']].
                                       event_to_token[f'Tempo_{current_tempo}'])
                tokens.append(track_token)

            # Adding bar and position tokens to notes for positional encoding
            note_token[0] = self.vocab[0].event_to_token[f'{note_token[0].type}_{note_token[0].value}']
            note_token.insert(1, self.vocab[1].event_to_token[f'BarPosEnc_{current_bar}'])
            note_token.insert(2, self.vocab[2].event_to_token[f'PositionPosEnc_{current_pos}'])
            if self.additional_tokens['Tempo']:
                note_token.insert(3, self.vocab[self.vocab_types_idx['Tempo']][f'Tempo_{current_tempo}'])
        ...
        for note in track.notes:
            ...
            if not track.is_drum:
                tokens.append([Event(type_='Pitch', value=note.pitch, time=note.start, desc=track.program),
                               ...
                               self.vocab[-1][f'Duration_{".".join(map(str, self.durations[dur_idx]))}']])
            else:
    ...
    def tokens_to_midi(self, tokens: List[List[int]], _=None, output_path: Optional[str] = None,
        ...
        for time_step in tokens:
            ...
            if tok_type == 'Bar':
                ...
                vel, duration = (self.vocab[i].token_to_event[time_step[i]].split('_')[1] for i in (-2, -1))
    ...
    def _create_vocabulary(self, sos_eos_tokens: bool = None) -> List[Vocabulary]:
        ...
        vocab = [Vocabulary(pad=self._pad, sos_eos=self._sos_eos, mask=self._mask) for _ in range(3)]
        ...
        # POSITION POS ENC
        vocab[2].add_event('PositionPosEnc_None')  # special embedding used with 'Bar_None' tokens
        vocab[2].add_event(f'PositionPosEnc_{i}' for i in range(nb_positions))  # pos enc
        ...
        vocab += [Vocabulary(pad=self._pad, sos_eos=self._sos_eos, mask=self._mask) for _ in range(2)]

        # VELOCITY
        vocab[-2].add_event(f'Velocity_{i}' for i in self.velocities)

        # DURATION
        vocab[-1].add_event(f'Duration_{".".join(map(str, duration))}' for duration in self.durations)
